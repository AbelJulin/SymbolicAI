...
public class ExpertActionStrategy implements ActionStrategy {
    ...
    // Le thread pour réfléchir pendant le tour de l'adversaire (Pondering)
    private Thread ponderThread = null;

    /**
     * Initialisation de l'IA avec les outils de stats pour le débug.
     */
    public ExpertActionStrategy(AlphaBetaPruningObserver observateur,
    ...
    public Action selectAction(Board board, int playerId) {
        // Arrêter la réflexion en background avant de reprendre le contrôle de la mémoire
        ...
        if (meilleurCoupTrouve == null) {
            ...
            meilleurCoupTrouve = actionsDisponibles.get(0);
        }

        // --- DEBUT PONDERING ---
        // On clone le plateau actuel pour simuler notre coup
        ...
        if (!clonedBoard.isFinished()) {
            ponderThread = new Thread(() -> {
                try {
                    int actionsSize = clonedBoard.getSortedActions().size();
                    int depth = 1;
                    // On donne un temps virtuellement infini pour chercher tant qu'on n'est pas interrompu
                    long ponderDeadline = System.currentTimeMillis() + 86400000L; // 1 jour
                    int scoreInitial = clonedBoard.getScore(playerId) - clonedBoard.getScore(1 - playerId);

                    while (depth <= actionsSize && !Thread.currentThread().isInterrupted()) {
                        alphaBetaTT(clonedBoard, depth, nextPlayerId, playerId, scoreInitial,
                                Integer.MIN_VALUE, Integer.MAX_VALUE, ponderDeadline);
                        depth++;
                    }
                } catch (TimeOutException ignored) {
                    // C'est le signal normal d'arrêt quand on est interrompu
                } catch (Exception ignored) {
                }
            });
            ponderThread.setDaemon(true); // Pour ne pas bloquer l'arrêt du programme
            ponderThread.start();
        }
        // --- FIN PONDERING ---
