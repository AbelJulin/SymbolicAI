...
public class HeuristicFunction {
    ...
    public static int evaluate(Board board, int playerId) {
        ...
        if (longStructures > 0) {
            // Compter les coups sûrs (ni closing ni giving) — sans allouer getAvailableActions
            int safeMoveCount = countSafeMoves(board, rows, cols);

            // Si pair → le joueur courant devra ouvrir la première chaîne
            // Si impair → l'adversaire devra l'ouvrir
            boolean weOpenFirst = (safeMoveCount % 2 == 0);

            // On veut NE PAS être celui qui ouvre
            ...
            if (chainCount > 0) {
                // Tri insertion (petit tableau, pas besoin de sort complexe)
                ...
                for (int i = 0; i < chainCount; i++) {
                    ...
                    if (i < chainCount - 1 && sz > 2) {
                        netGain += (sz - 2); // double-dealing : sacrifie 2
                    } else {
                        netGain += sz; // dernière chaîne : tout prendre
                    }
                }
        ...
        // Bonus cases capturables
        value += capturableBoxes * chainWeight;

        // Pénalité chaînes longues (tension)
        ...
        value -= dangerWeight * dangerousBoxes;
